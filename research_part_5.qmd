...
df_technical_violations_dispo_clean_demo <- read_delim(file.path(sharepoint_jj_av_dallas_analytic_data,
  ...
  ### re-code disposition of "DISMISSED OR WITHDRAWN BY DEPARTMENT" as "CAI" when cai flag is 1
  ### otherwise, we will group it with the other dismissed/withdrawn category
    mutate(violation_disposition = case_when(
      violation_disposition=="DISMISSED OR WITHDRAWN BY DEPARTMENT" & cai_program_flag==1 ~ "CAI",
      violation_disposition=="DISMISSED OR WITHDRAWN BY DEPARTMENT" & is.na(cai_program_flag)==TRUE ~ "DISMISSED OR WITHDRAWN BY DEPARTMENT",
      TRUE ~ as.character(violation_disposition))) |> 
    mutate(violation_dispo_clean = 
           case_when(
             ...
             violation_disposition=="CAI" ~ "21 Day Residential Program",
             violation_disposition%in%c("COMPLAINT DISMISSED BY COURT",
                            ...
                            "DISMISSED OR WITHDRAWN BY DEPARTMENT") ~ "Dismissed/Withdrawn",
...
df_complaints_youth_intake_decision_offense_viz_overall_count <- df_technical_violations_dispo_clean_demo %>% 
  ...
  dplyr::select(-group) |> 
  mutate(violation_filed_date_fy = as.numeric(violation_filed_date_fy))
...
df_technical_violations_overall <- df_technical_violations_dispo_clean_demo %>% 
  ...
  mutate(group = "dallas") |> 
  pivot_longer(!group, names_to = "complaint_type", values_to = "percent") 
...
*	1. How should we interpret the violation dispositions of ““DISMISSED OR WITHDRAWN BY DEPARTMENT” without any “emergency shelter” placement records? Are these potentially supervisor rejections?

*	2. How should we interpret the placement records with “Emergency Shelter” and “VIOL OF COURT ORDER  - TECHNICAL”, but that don’t join to the violations file? What are other examples of “Emergency Shelter” placements that are linked to a technical violation? 
...
unique_violations_sample_denom <- n_distinct(df_technical_violations_dispo_clean_demo$youth_violation_id)

### unique youth and referrals per FY
df_dismissed_violations_per_year <- df_technical_violations_dispo_clean_demo %>% 
  dplyr::group_by(violation_filed_date_fy) %>% 
  dplyr::summarise(`Unique Youth w/ at least One Violation Disposed to 21 Day Program (N)` = n_distinct(unique_id[violation_dispo_clean=="21 Day Residential Program"]),
                   `Technical Violations Disposed to 21 Day Program (N)` = n_distinct(youth_violation_id[violation_dispo_clean=="21 Day Residential Program"]),
...
### overall
df_violations_overall <- df_technical_violations_dispo_clean_demo %>% 
  dplyr::summarise(`Unique Youth w/ at least One Violation Disposed to 21 Day Program (N)` = n_distinct(unique_id[violation_dispo_clean=="21 Day Residential Program"]),
                   `Technical Violations Disposed to 21 Day Program (N)` = n_distinct(youth_violation_id[violation_dispo_clean=="21 Day Residential Program"]),
            `All Technical Violations (N)` = n_distinct(youth_violation_id),
            `Technical Violations Disposed to 21 Day Program (row %)` = scales::percent(`Technical Violations Disposed to 21 Day Program (N)`/`All Technical Violations (N)`,
